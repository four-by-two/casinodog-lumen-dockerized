...
class GameRespinTemplate extends Eloquent  {
    ...
    {   
        if($game_type === 'normal') {
            if(GameRespinTemplate::where('gid', $gid)->where('game_type', $game_type)->count() > 10) {
                return true;
            }
        }
    ...
    {
        try {
        $game = collect(GameRespinTemplate::all()->where('gid', $gid)->where('game_type', $game_type)->random(1));
